...
public class FileService extends ServiceImpl<FileMapper, File> {
    ...
    private String createUploadId(){
        ...
        LOGGER.info("上传文件初始化完成，uploadId：{}", uploadId);
        return uploadId;
    }
     ...
     * 
     * 【原理讲解】
     * 1. sumTotalFileSize(): 执行 SQL SUM 聚合，统计所有文件大小
     * 2. countFiles(): 统计文件数量
     * 3. maxSize: 配置的最大存储空间（默认 10GB）
     * 4. 返回 Map 包含：usedSize(已用)、fileCount(文件数)、maxSize(最大)、usedPercent(百分比)
     */
    public Map<String, Object> getStorageStats() {
        Map<String, Object> stats = new HashMap<>();
        
        // 统计已用空间（字节）
        Long usedSize = fileMapper.sumTotalFileSize();
        // 统计文件数量
        Long fileCount = fileMapper.countFiles();
        // 最大空间限制（10GB，可配置）
